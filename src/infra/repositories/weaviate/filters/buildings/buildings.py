...
def append_building_facility_filters(
    buildings_filter: BuildingsFilter, filter_array: list
) -> list:
    if isinstance(buildings_filter.building_facility, str):
        result_list = [
            item.strip() for item in buildings_filter.building_facility.split(",")
        ]
        if all(isinstance(item, str) for item in result_list):
            for k in result_list:
                filter_array.append(
                    Filter.by_ref(link_on="hasBuilding")
                    .by_property("buildingDescription")
                    .like(f"*{k}*")
                )
...
def append_building_note_filters(
    buildings_filter: BuildingsFilter, filter_array: list
) -> list:
    if isinstance(buildings_filter.building_note, str):
        result_list = [
            item.strip() for item in buildings_filter.building_note.split(",")
        ]
        if all(isinstance(item, str) for item in result_list):
            for k in result_list:
                filter_array.append(
...
def append_building_geolocation_filter(lat_long: str, distance: float) -> list:
    if lat_long is not None:
        return Filter.by_property("buildingGeolocation").within_geo_range(
