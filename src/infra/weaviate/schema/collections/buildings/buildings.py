...
def append_housing_price_filters(buildings_filter: BuildingsFilter, filter_array: list) -> list:
    if(isinstance(buildings_filter.less_than_price, float) and \
        ...
        filter_array.append(Filter.by_property("housing_price").greater_or_equal(buildings_filter.greater_than_price))
        filter_array.append(Filter.by_property("housing_price").less_or_equal(buildings_filter.less_than_price))
...
def append_property_address_filters(buildings_filter: BuildingsFilter, filter_array: list) -> list:
    if(buildings_filter.building_address is not None):
        filter_array.append(Filter.by_property("property_address").contains_any([buildings_filter.building_address]))
    return filter_array
...
def append_property_title_filters(buildings_filter: BuildingsFilter, filter_array: list) -> list:
    if(buildings_filter.building_title is not None):
        filter_array.append(Filter.by_property("property_title").contains_any([buildings_filter.building_title]))
    return filter_array
